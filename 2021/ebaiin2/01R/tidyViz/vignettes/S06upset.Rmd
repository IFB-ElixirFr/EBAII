...
Nous aurons besoin des packages `ggvenn` et `ComplexUpset` :

 * Vérifier que les packages `ggvenn` et `ComplexUpset` sont bien installés
 * Si non, les installer, puis les charger

```{r ComplexUpset, message = FALSE}
library(ggvenn)
...
upset(fbin, intersect = c("Sucres", "Fibres", "Energie", "Potassium", "Crus"))
```
