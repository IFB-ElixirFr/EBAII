...
params:
  ...
  <!-- rnaseq: /Users/elodiedarbo/Documents/enseignement/EBAIIv2_2021/data/RNAseq/RNAseq_diff_norm.RData -->
...
#knitr::opts_knit$set(echo = TRUE,  root.dir="/Users/elodiedarbo/Documents/enseignement/EBAIIv2_2021/", fig.path = "images/")
```
...
```{r enhancedbarplot, include=TRUE, out.width="50%"}
...
```{r enhancedbarplotfollow, include=TRUE, fig.show="hold", out.width="50%", fig.align = 'center'}
...
Sometime, peaks may occur more in some chromosomes than others. We can display the genomic distribution of the peaks along the chromosomes, using the `covplot` function from `ChIPSeeker`. Height of peaks is drawn based on the peak scores.
...
We can now perform a more detailled analysis of genomic features associated to our peaks (introns, exons, promoters, distal regions,...).
...
First of all, for calculating the profile of ChIP peaks binding to the BRG1 center, we need to define peak centers and extend them each side. Then (we need to) align the peaks that are mapped to these regions, and to generate the `tagMatrix`.

```{r signalAroundBRG1center, message=FALSE, include=TRUE, cache = TRUE, fig.align = 'center'}
## Load the library
library(GenomicRanges)
...
We can display a summary of the binding profiles (by) looking at the corresponding average profile. This kind of profiles is much less greedy, we can (thus) extend a bit more (e.g. +/- 5000) from the peak centers redoing the previous steps. Definition of the regions have to be redone. Try to create the `GRanges` and the `tagMatrix`. \
...
We can now create a matrix of read enrichment at the positions of interest. We are using functions from `ChIPpeakAnno` package.
...
```{r Heatmap, message=FALSE, include=TRUE, cache = TRUE, fig.align = 'center'}
## Load the library
library(EnrichedHeatmap)
...
```{r Kmeans, message=FALSE, include=TRUE, cache = TRUE, fig.align = 'center'}
...
```{r clusterKmeans, warning=F,message=F, cache = TRUE, fig.align = 'center'}
...
```{r nbPeaks, include=TRUE, fig.align = 'center'}
# Load ggplot2 library
library(ggplot2)
peak.lengths <- data.frame(IP=names(peaks),
                           NbPeaks=sapply(peaks,length))
...
```{r peakWidthNice, message=FALSE, include=TRUE, fig.align = 'center'}
ggplot(peak.width.table, aes(x=L1, y=value, fill=L1)) +
         geom_boxplot()+
         theme_classic()+
         scale_fill_brewer(palette="Set1")+
...
```{r TSSheatmapKmeans, message=F,warning=F, cache = TRUE, fig.align = 'center'}
# Define new colors for each heatmap
...
[Représentation simplifiée graphique du test hypergéometrique](hypergeometric.png)
...
```{r funcEnrcl4, message=F,warning=F, cache = TRUE}
# load the library
library(pathview)
