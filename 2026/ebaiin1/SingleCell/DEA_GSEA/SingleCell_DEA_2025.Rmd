...
<center>![](images/eb3i_banner.png)</center>
...
```{r setparam, eval=FALSE, include=FALSE}
# setparam
...
```{r maindir,eval=FALSE, include=FALSE}
#maindir
...
```{r sessiondir,eval=FALSE, include=FALSE}
# sessiondir
...
```{r indir, eval=FALSE, include=FALSE}
#indir
...
```{r outdir, eval=FALSE, include=FALSE}
#outdir
...
```{r dataload, eval=FALSE, include=FALSE}
##  dataload
...
```{r not_show_loading_data, include=FALSE}

sobj <- readRDS("05_DEA/RESULTS/12_TD3A.TDCT_S5_Integrated_12926.3886.RDS")
...
```{r sobj_mast_res_display, eval=TRUE, echo=FALSE}
DT::datatable(
  head(sobj_mast, n = 10),
...
data <- base::list(
  ...
  mast    = base::rownames(sobj_mast[sobj_mast$p_val_adj <= 0.05, ]),
  # We use a threshold of 5% on adjusted p-values
