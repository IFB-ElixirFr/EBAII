---
title: "ChIP-seq workshop - Hands-on Roscoff 2025"
author: "Elodie Darbo; Stéphanie Le Gras; Pascal Martin;"
date: "2025 November 18-20"
output:
  html_document:
    fig_caption: yes
    toc: yes
...
1. Click on **SRR576933** in SRA 
2. There are statistics on the run that generated the data. 
3. Click on FASTA/FASTQ download. On the next page, there is a link to the FASTQ file. For efficiency, this file has already been downloaded.
...
**Samples have been renamed to have explicite names**:
  * FNR_IP_ChIP-seq_Anaerobic_A.fastq.gz (SRR576933)
  * FNR_IP_ChIP-seq_Anaerobic_B.fastq.gz (SRR576934)
  * Anaerobic_INPUT_DNA.fastq.gz (SRR576938)
...
```{bash eval=FALSE}
mkdir EBAII2025_chipseq
```
3. Go to the newly created directory
```{bash eval=FALSE}
cd EBAII2025_chipseq
```
4. Copy the directory containing data

```{bash eval=FALSE}
cp -r /shared/projects/2538_eb3i_n1_2025/atelier_chipseq/training_data/data .
```

7. Your directory structure should be like this
 ```
/shared/projects/<your_project>/EBAII2025_chipseq
│
└───data
```
...
7. Go to the directory /shared/projects/\<your_project\>/EBAII2025_chipseq/1-QualityControl in the tree directory on the left of the jupyterhub window and double click on FNR_IP_ChIP-seq_Anaerobic_A_fastqc.html to visualize the file.
![alt text][fastqc]
...
```{bash eval=FALSE}
cd /shared/projects/<your_project>/EBAII2025_chipseq/02-Mapping/bam
```
...
setwd("/shared/projects/<your_project>/EBAII2025_chipseq/07-PeakAnnotation")
...
```{bash eval=FALSE}
cd /shared/projects/<your_project>/EBAII2025_chipseq
...
setwd("/shared/projects/<your_project>/EBAII2025_chipseq/07-PeakAnnotation-bonus")
