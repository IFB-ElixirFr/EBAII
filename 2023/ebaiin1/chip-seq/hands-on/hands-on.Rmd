---
title: "ChIP-seq workshop - Hands-on Roscoff 2023"
author: "Elodie Darbo; Stéphanie Le Gras; Rachel Legendre; Denis Puthier; Morgane Thomas-Chollier; Tao Ye;"
date: "2023 Novembre 7-9"
output:
  html_document:
    fig_caption: yes
    toc: yes
...
```{bash eval=FALSE}
mkdir EBAII2023_chipseq
```
3. Go to the newly created directory
```{bash eval=FALSE}
cd EBAII2023_chipseq
```
4. Copy the directory containing data

```{bash eval=FALSE}
cp -r /shared/projects/2325_ebaii/chipseq/EBAII2023_chipseq/data .
```

7. Your directory structure should be like this
 ```
/shared/projects/<your_project>/EBAII2023_chipseq
│
└───data
```
...
7. Go to the directory /shared/projects/<your_project>/EBAII2023_chipseq/1-QualityControl in the tree diretory on the left of the jupyterhub panel and double click on SRR576933_fastqc.html to visualize the file.
...
```{bash eval=FALSE}
cd /shared/projects/<your_project>/EBAII2023_chipseq/02-Mapping/bam
```
...
Go back to working home directory (i.e /shared/projects/<your_project>/EBAII2023_chipseq/)
```{bash eval=FALSE}
...
```{bash eval=FALSE}
cp /shared/home/slegras/EBAII2023_chipseq/03-ChIPQualityControls/fingerprint.png .
```
...
Go back to working home directory (i.e /shared/projects/training/\<login\>/EBAII2023_chipseq)
```{bash eval=FALSE}
## Unload deepTools
...
setwd("/shared/projects/<your_project>/EBAII2023_chipseq/07-PeakAnnotation")
...
```{bash eval=FALSE}
tail -n +2 FNR_anaerobic_idr_final_peaks_annotation.tsv | awk -F "\t" '{print $10}'
```
...
tail -n +2 FNR_anaerobic_idr_final_peaks_annotation.tsv | awk -F "\t" '{print $7}' | sort | uniq -c
```

How many peaks are associated with promoters ?
```{bash eval=FALSE}
tail -n +2 FNR_anaerobic_idr_final_peaks_annotation.tsv | awk -F "\t" '{if ($7=="promoter-TSS") print $10}'
```

**Is the number of genes in your file consistent with the previous reply?**

```{bash eval=FALSE}
tail -n +2 FNR_anaerobic_idr_final_peaks_annotation.tsv | awk -F "\t" '{if ($7=="promoter-TSS") print $10}' | wc -l
tail -n +2 FNR_anaerobic_idr_final_peaks_annotation.tsv | awk -F "\t" '{if ($7=="promoter-TSS") print $10}' \
...
```{bash eval=FALSE}
cd /shared/projects/<your_project>/EBAII2023_chipseq
