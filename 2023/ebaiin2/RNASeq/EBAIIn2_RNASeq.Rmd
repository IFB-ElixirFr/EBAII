...
res_4vs0 <- results(dds, contrast=c("time", "T4", "T0"))
res_8vs0 <- results(dds, contrast=c("time", "T8", "T0"))
res_8vs4 <- results(dds, contrast=c("time", "T8", "T4"))
...
$$ \text{OR} = \frac{\text{Odd in gene-set}}{\text{Odd out gene-set}} = \frac{25/113}{1152/18047} = 3.46 $$
est significativement supérieur à 1 (P-valeur = $8.25 \times 10^{-7}$). On peut également calculer le *Fold-Enrichment* :

$$ \text{FE} = \frac{\text{Pct gene DE in gene-set}}{\text{Pct gene DE out gene-set }} = \frac{25/(25+113)}{1152/(1152+18047)} = \frac{0,181}{0,6} = 3,02$$
...
```{r fgsea_illustration2, fig.height=10, fig.width=14, dpi=300, out.width="100%"}
...
Dans les sections précédentes, nous avons utilisé des *pathways* KEGG importés à partir du package R gage. Il est également possible de les importer en utilisant le package msigdbr, lui même basé sur ces [données](https://www.gsea-msigdb.org/gsea/msigdb/index.jsp) :
